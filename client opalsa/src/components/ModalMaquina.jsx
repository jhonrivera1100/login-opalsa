    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-black bg-opacity-50">
      <div className="bg-sky-100 shadow-xl w-full max-w-4xl max-h-[80vh] overflow-y-auto p-6 relative text-sm">
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex mb-6 h-1/2 bg-slate-600 p-4 rounded-lg relative"> {/* Fondo común y padding */}
            <div className="w-1/3 pr-4 flex flex-col items-center justify-center relative">
              <div className="absolute top-0 left-0 w-[90%] bg-gray-900 text-white text-center py-1 rounded-t-lg"> {/* Anchura ajustada */}
                <strong>Serial: {maquina.nroSerieMaquina}</strong> {/* Franja Número de Serie */}
              </div>
              <img
              ...
              />
              <div className="absolute bottom-0 left-0 w-[90%] bg-gray-900 text-white text-center py-1 rounded-b-lg mx-auto"> {/* Anchura ajustada */}
                <strong>Marca: {maquina.marcaMaquina}</strong> {/* Franja Marca */}
              </div>
            </div>
            <div className="w-2/3 pl-4 overflow-auto text-white">
              <h2 className="text-xl font-bold mb-2">Datos de la maquina:</h2>
              <div className="space-y-0.5">
              <p><strong className="text-sky-300">Nombre:</strong> {maquina.nombreMaquina}</p>
                <p><strong className="text-sky-300">Modelo:</strong> {maquina.modeloMaquina}</p>
                <p><strong className="text-sky-300">Software:</strong> {maquina.softwareMaquina}</p>
                <p><strong className="text-sky-300">Juego:</strong> {maquina.juegoMaquina}</p>
                <p><strong className="text-sky-300">Estado:</strong> {maquina.estadoMaquina}</p>
                <p><strong className="text-sky-300">Descripción:</strong> {maquina.descripcionMaquina}</p>
                <p><strong className="text-sky-300">Ubicación:</strong> {maquina.ubicacionMaquina}</p>
                <p><strong className="text-sky-300">Fecha de Instalación:</strong> {maquina.fechaInstalacionMaquina}</p>
                <p><strong className="text-sky-300">Proveedor:</strong> {maquina.proveedorMaquina}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-auto">
            ...
            >
              ...
              <svg 
                xmlns="http://www.w3.org/2000/svg" 
                className={`h-6 w-6 transform transition-transform ${showComponentes ? 'rotate-180' : ''}`} // Rotación
                fill="none" 
                viewBox="0 0 24 24" 
                stroke="currentColor"
              >
              ...
              <>
                <ul className="list-none pl-0">
                  ...
                  {sortedComponentes.map((componente) => (
                    <li className="text-gray-500 mb-1 grid grid-cols-4 gap-4 font-medium p-2 border-b border-gray-300" key={componente._id}>
